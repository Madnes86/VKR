...
<Flex col className="items-start">
	<div
		...
		class={`${state == 'error' && 'outline outline-red-400'} m-1 flex w-full items-center rounded-md`}
		class:bg-black={state == 'edit' || state == 'error'}
	>
		<Button
			{disabled}
			onclick={() => (active = !active)}
			className="p-1 flex w-full rounded-md gap-2 hover:bg-[#323232]"
		>
			<div class="size-6">
			...
			{:else if state == 'edit' || state == 'error'}
				...
				<input
					...
					autofocus
					class={`${state == 'error' && ''} h-6 w-full border-0 bg-transparent p-0 focus:outline-none`}
		...
		{#if hover || state == 'error' || state == 'edit'}
			{#snippet button(name: string, onclick: () => void, stroke?: string)}
				<button
					...
					{onclick}
					disabled={saving}
					class="click ml-1 rounded-md p-2 hover:bg-[#323232]"
				>
					<Icon {name} {stroke} />
				</button>
			{/snippet}
