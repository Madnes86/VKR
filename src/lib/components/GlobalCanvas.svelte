<script lang="ts">
    ...
    function onmouseup() { draggedId = null; panning = false; }
    ...
    function hover(n: Node) { selectedStore.set('hover', `o + ${n.id}`); }
    ...
    function isSelected(id: number) { return selectedStore.selected === `o + ${id}`; }
    function isHover(id: number) { return selectedStore.hover === `o + ${id}`; }
</script>
...
<div
    ...
    class="relative h-screen w-full overflow-hidden select-none bg-zinc-950 {panning ? 'cursor-grabbing' : 'cursor-grab'}"
>
    <div
    ...
    >
        ...
        >
        <svg class="absolute top-0 left-0 size-full pointer-events-none">
            {#each edges as e (e.id)}
                {@const isHighlighted = isHover(e.is.id) || isHover(e.to.id) || isSelected(e.is.id) || isSelected(e.to.id)}
                <line
        ...
        {#each visibleNodes as n (n.id)}
            ...
            {@const fontSize = Math.max(11, Math.min(n.size / 6, n.size * 0.9 / Math.max(4, n.name.length)))}
            ...
            <button
                ...
                class="absolute rounded-full bg-black border-2 flex items-center justify-center text-white cursor-grab active:cursor-grabbing transition-colors
                    {isSelected(n.id) ? 'border-accent' : isHover(n.id) ? 'border-accent/70' : 'border-white'}"
            >
                ...
                {#if cluster}
                    <span
                        style="width: {badgeSize}px; height: {badgeSize}px; font-size: {Math.max(9, fontSize * 0.55)}px"
    ...
    <div
        ...
        class="absolute bottom-2 right-2 z-20 flex items-center gap-1 bg-black/60 rounded-md px-2 py-1 text-xs text-white font-mono select-none"
    >
        <button onclick={() => scaleStore.zoomOut()} class="click px-2 hover:text-accent" aria-label="zoom-out">−</button>
        <span>{Math.round(scaleStore.value * 100)}%</span>
        <button onclick={() => scaleStore.zoomIn()} class="click px-2 hover:text-accent" aria-label="zoom-in">+</button>
