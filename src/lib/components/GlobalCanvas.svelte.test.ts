...
describe('GlobalCanvas', () => {
    ...
    function setup() { render(GlobalCanvas); }
    ...
    it('панорамирование: mousedown на пустой области + mousemove сдвигают камеру', async () => {
        ...
        container.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, clientX: 100, clientY: 100, button: 0 }));
    ...
    it('при появлении на зуме дети помещаются внутрь круга родителя', async () => {
        ...
        for (const name of ['obj1', 'obj4']) {
            ...
            expect(dist, `${name} distance ${dist} should fit inside root radius ${maxAllowed}`).toBeLessThanOrEqual(maxAllowed);
        }
    });
