<script lang="ts">
	...
	const tokens: Token[] = $derived.by(() => {
		...
		for (const part of parts) {
			if (!part) continue;
			if (/^\s+$/.test(part)) {
				result.push({ kind: 'space', text: part });
				continue;
			}
			const o = items.find((x) => x.name === part);
			if (o) {
				result.push({ kind: 'object', text: part, id: o.id });
				continue;
			}
			const l = linkItems.find((x) => x.name === part);
			if (l) {
				result.push({ kind: 'link', text: part, id: l.id });
				continue;
			}
...
<div class="flex size-full h-screen flex-col border-l-2 border-gray">
	<div class="flex items-center gap-2 pr-2">
		...
		<button
			onclick={sync}
			class="click flex items-center gap-2 rounded-md bg-accent px-3 py-1 hover:brightness-110"
		>
			<Icon name="refresh" />
		...
		<div class="relative w-full font-mono text-base leading-relaxed">
			{#if !filterMode}
				<textarea
					...
					class="absolute inset-0 size-full resize-none rounded-md bg-zinc-950 p-3 break-words whitespace-pre-wrap text-transparent caret-accent outline-none"
				></textarea>
			{/if}
			<div
				class="pointer-events-none relative p-3 break-words whitespace-pre-wrap {filterMode
					? 'min-h-full rounded-md bg-zinc-950'
					: ''}"
			>
				{#each displayTokens as t, i (i)}
					{#if t.kind === 'object'}
						...
						<span
							class="pointer-events-auto -mx-1 cursor-pointer rounded-sm px-1 text-white transition-colors hover:bg-accent"
					...
					{:else if t.kind === 'link'}
						<span
							class="pointer-events-auto -mx-1 rounded-sm px-1 text-zinc-400 italic transition-colors hover:bg-accent"
							>{#if searchLinks}<LightText text={t.text} />{:else}{t.text}{/if}</span
						>
	...
	<div class="my-2 flex gap-4 px-3 py-1 font-mono text-xs text-zinc-400 select-none">
		...
		<span
			>{i18n.t('global.stats.unsynced')}:
			<span class={unsyncedCount > 0 ? 'text-accent' : 'text-white'}>{unsyncedCount}</span></span
		>
		<span class="ml-auto"
			>{i18n.t('global.stats.chars')}: <span class="text-white">{text.length}</span></span
		>
	</div>
</div>
