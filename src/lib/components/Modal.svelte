<script lang="ts">
    ...
    import { register, login, logout, updateMe, startOAuth, fetchProviders, requestPasswordReset, type OAuthProvider } from "$lib/functions/auth";
        ...
        <div bind:this={ref} class="size-100 p-2 flex gap-2 flex-col">
            {#snippet oauthButtons()}
                {#if providers.google || providers.yandex}
                    <div class="flex gap-3 w-full justify-center p-2">
                        {#if providers.google}
                            <button onclick={() => startOAuth('google')} class="click size-8 rounded-full overflow-hidden">
                                <img src={googleIcon} alt="Google" class="w-full">
                            </button>
                        {/if}
                        {#if providers.yandex}
                            <button onclick={() => startOAuth('yandex')} class="click size-8 rounded-full overflow-hidden">
            ...
            {#if modalStore.type == 'login'}
                ...
                <button onclick={goResetRequest} class="click text-sm text-gray-300 hover:text-accent">{i18n.t('modal.forgotPassword')}</button>
                {@render oauthButtons()}
            ...
            {:else if modalStore.type == 'logout'}
                ...
                <div class="flex gap-2">
                    <button onclick={close} class="click rounded-sm p-2 flex-1 border border-gray">{i18n.t('common.cancel')}</button>
                    <button onclick={logout} class="click rounded-sm p-2 flex-1 bg-red text-white">{i18n.t('modal.leave')}</button>
                </div>
            {:else if modalStore.type == 'password'}
                <h1>{i18n.t('modal.changePassword')}</h1>
                <Form2 bind:value={currentPassword} icon="password" label={i18n.t('modal.currentPassword')} iType="password" />
                <Form2 bind:value={password} icon="password" label={i18n.t('modal.newPassword')} validate={passwordRule} iType="password" />
                <div class="flex gap-2 mt-4">
                    <button onclick={close} class="click rounded-lg p-1 flex-1 border border-gray">{i18n.t('common.cancel')}</button>
                    <button onclick={submitPassword} class="click rounded-lg p-1 flex-1 bg-accent">{i18n.t('modal.save')}</button>
                </div>
            {:else if modalStore.type == 'resetRequest'}
                ...
                <Form2 bind:value={mail} icon="mail" label={i18n.t('common.email')} validate={emailRule} />
                <div class="flex gap-2 mt-4">
                    <button onclick={goLogin} class="click rounded-lg p-1 flex-1 border border-gray">{i18n.t('common.cancel')}</button>
                    <button onclick={submitResetRequest} class="click rounded-lg p-1 flex-1 bg-accent">{i18n.t('modal.sendResetLink')}</button>
                </div>
            {:else if modalStore.type == 'registration'}
