<script lang="ts">
	...
	let drops = $state([
		{
			items: ['EN', 'RU'],
			selected: i18n.lang.toUpperCase(),
			name: 'lang',
...
<div class="flex w-full flex-col items-stretch gap-2 p-2">
	...
	<div class="flex w-full items-center gap-3 px-2 py-2">
		...
		<div class="flex min-w-0 flex-col">
			...
			{#if isAuth}
				<span class="truncate text-xs opacity-60">{email}</span>
			{:else}
	...
	{#each drops as { items, name, labelKey }, i}
		<DropDown {items} bind:selectedItem={drops[i].selected} className="w-full">
			<div class="click flex w-full items-center gap-3 rounded-sm px-2 py-1 hover:bg-gray">
				<Icon {name} />
				<p>{i18n.t(labelKey)}</p>
	...
	<div class="flex items-center gap-2 border-b border-gray px-2 pt-2 pb-1">
		<Icon name="diagramm" />
		<span class="grow font-semibold">{i18n.t('settings.diagram.title')}</span>
	</div>
	<div class="flex flex-col gap-1">
		...
		{#if isModified}
			<Button
				onclick={() => diagramSettings.reset()}
				className="flex gap-2 p-1 m-1 w-full hover:bg-gray rounded-md text-sm opacity-80"
			>
				<Icon name="cross" />
	...
	<div class="flex items-center gap-2 border-b border-gray px-2 pt-2 pb-1">
		<Icon name="mouse" />
		<span class="grow font-semibold">{i18n.t('settings.shortcuts.title')}</span>
	</div>
	<div class="flex flex-col gap-1 px-2">
		{#each shortcuts as s}
			<div class="flex items-center justify-between gap-2 py-1 text-sm">
				<span class="grow opacity-80">{i18n.t(s.labelKey)}</span>
				<span class="flex gap-1">
					{#each s.keys as k}
						<kbd class="rounded border border-gray bg-gray-glass px-1.5 py-0.5 text-xs">{k}</kbd>
					{/each}
				</span>
			</div>
		{/each}
	...
	{#if isAuth}
		...
		<Button
			...
			className="flex gap-2 p-1 m-1 w-full hover:bg-gray rounded-md"
		>
		...
		<Button
			onclick={openLogout}
			className="flex gap-2 p-1 m-1 text-red w-full hover:bg-gray rounded-md"
		>
			<Icon name="exit" />
	...
	{:else}
		<Button
			onclick={openLogin}
			className="flex gap-2 p-1 m-1 text-accent w-full hover:bg-gray rounded-md"
		>
			<Icon name="login" />
