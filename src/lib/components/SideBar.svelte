...
{#if show}
	<div
		class="{posClass} {right
			? 'flex-row-reverse'
			: 'flex-row'} absolute top-0 z-1 flex h-screen backdrop-blur-xs"
	>
		...
		<span {onmousedown} class="h-screen w-0.5 cursor-col-resize bg-gray hover:bg-accent"></span>
	</div>
	{#if 2 > side.v.length}
		<div
			{ondrop}
			{ondragover}
			{ondragleave}
			class="{hover ? 'border-accent bg-gray-glass' : 'border-transparent'} {right
				? 'left-0 border-r-2'
				: 'right-0 border-l-2'} fixed top-0 z-10 h-screen w-40 border-dashed"
		></div>
	{/if}
{:else}
	<div class="{posClass} absolute top-0 z-1 flex p-1 backdrop-blur-xs">
