...
function tnode(id: number, name = `n${id}`, children: ITreeObject[] = [], links: IFlatLink[] = []): ITreeObject {
    return {
        id, name, type: 'default', parent: null,
        x: 0, y: 0, size: 100, mass: 1,
        objects: children, links,
...
describe('pruneTree', () => {
    ...
    it('фильтрует links того уровня, на котором они привязаны', () => {
        const tree = tnode(0, 'root', [tnode(1), tnode(2), tnode(3)], [
            lnk(10, 1, 2),
            lnk(11, 1, 3),
        ]);
        const visible = new Set([1, 2]);
