 ...
 */
export function computeSearchVisibility(objects: IFlatObject[], query: string): Set<number> {
 ...
 */
export function filterLinksByVisibility(links: IFlatLink[], visible: Set<number>): IFlatLink[] {
...
export function pruneTree(node: ITreeObject, visible: Set<number>): ITreeObject {
	...
	const links = (node.links ?? []).filter((l) => visible.has(l.is) && visible.has(l.to));
