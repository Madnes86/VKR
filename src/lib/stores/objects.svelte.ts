...
function lsSet(key: string, value: string): void {
    ...
    try { localStorage.setItem(key, value); } catch { /* quota — игнорируем */ }
}
...
export class ObjectsStore {
    ...
    setAll(next: IFlatObject[]) { this.#objects = next; this.#persist(); }
    get(id: number) { return this.#objects.find(o => o.id === id) }
    clear() { this.#objects = []; this.#tmpCounter = -1; this.#persist(); }
...
export class LinksStore {
    ...
    setAll(next: IFlatLink[]) { this.#links = next; this.#persist(); }
    get(id: number) { return this.#links.find(l => l.id === id) }
    clear() { this.#links = []; this.#tmpCounter = -1; this.#persist(); }
